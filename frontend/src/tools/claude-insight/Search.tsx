...
import { SearchClaudeSessions } from '../../../wailsjs/go/main/App'
import type { claudeinsight } from '../../../wailsjs/go/models'
import { formatRelative, shortenProject } from './lib/format'
import { SessionDetail } from './SessionDetail'

type Hit = claudeinsight.SearchHit
type Result = claudeinsight.SearchResult
...
export function Search() {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setError('')
    SearchClaudeSessions(debounced)
      .then((r) => {
  ...
  return (
    <div className="flex h-64 flex-col items-center justify-center gap-2 text-center">
      ...
      <p className="max-w-md text-sm text-muted-foreground">
        输入关键词搜索所有会话中的消息内容。大小写无关,按时间倒序展示命中片段。
      </p>
    </div>
  )
}
...
/**
 * HighlightedText 把 snippet 中匹配 query 的子串包在 <mark> 里,大小写无关。
 */
