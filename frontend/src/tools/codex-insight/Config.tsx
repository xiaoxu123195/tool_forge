...
export function Config({ reloadToken }: Props) {
  const [active, setActive] = useState<FileKey>('AGENTS.md')
    ...
    <div className="mx-auto flex max-w-5xl flex-col gap-3 h-full overflow-hidden">
      <div className="flex items-center gap-2">
        <FileTabButton
          ...
          onClick={() => setActive('AGENTS.md')}
          icon={<FileText className="h-3.5 w-3.5" />}
          label="AGENTS.md"
        />
        <FileTabButton
          active={active === 'config.toml'}
...
function FileEditor({ fileName }: { fileName: FileKey }) {
  ...
  const save = async () => {
    ...
    try {
      setSaving(true)
      await WriteCodexConfigFile(fileName, content)
      setOriginal(content)
      setToast('已保存')
