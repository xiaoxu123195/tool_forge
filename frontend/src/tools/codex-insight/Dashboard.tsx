...
type Report = codexinsight.DashboardReport

interface Props {
  reloadToken: number
}

export function Dashboard({ reloadToken }: Props) {
  ...
  if (report.total_sessions === 0) return <EmptyCodexDir dir={report.codex_dir} />
    ...
    <div className="flex h-full flex-col items-center justify-center gap-3 text-center">
      ...
      <div className="space-y-1">
        ...
        <p className="max-w-md text-sm text-muted-foreground">
          ...
          下没有读到 JSONL 会话文件。如果你还没用过 Codex CLI 就会是这个状态。
        </p>
      </div>
    </div>
  )
  ...
  const totalTokens = useMemo(
    () =>
      report.tokens_by_model.reduce(
        (sum, m) =>
          sum + m.input_tokens + m.output_tokens + m.cached_tokens + m.reasoning_tokens,
        0
      ),
...
function CalendarHeatmap({ buckets }: { buckets: codexinsight.DailyBucket[] }) {
  const weeks = 26
  const days = weeks * 7
...
function TokensByModelTable({ rows }: { rows: codexinsight.ModelTokens[] }) {
  if (rows.length === 0) return null
  return (
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.model} className="border-t border-border">
                ...
                <td className="px-3 py-2 text-right font-mono text-xs">{r.sessions.toLocaleString()}</td>
                ...
                <td className="px-3 py-2 text-right font-mono text-xs">{formatTokens(r.cached_tokens)}</td>
                <td className="px-3 py-2 text-right font-mono text-xs">{formatTokens(r.reasoning_tokens)}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
function LongestSessionBanner({ s }: { s?: codexinsight.SessionSummary }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-x-5 gap-y-2 rounded-lg border border-border bg-gradient-to-r from-info/10 to-transparent px-4 py-3">
      ...
      {s.model && (
        <BannerMetric label="模型" value={s.model} />
      )}
...
function RecentSessionsList({ list }: { list: codexinsight.SessionSummary[] }) {
